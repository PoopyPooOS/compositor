...
impl<T: AsFd> fmt::Debug for Device<T> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("Device").field("ptr", &format_args!("{:p}", &self.ffi)).finish()
    }
}
...
impl<T: AsFd> Device<T> {
    ...
    pub fn format_modifier_plane_count(&self, format: Format, modifier: Modifier) -> Option<u32> {
        unsafe {
            ffi::gbm_device_get_format_modifier_plane_count(*self.ffi, format as u32, modifier.into())
                .try_into()
                .ok()
        }
    }
    ...
    pub fn create_surface<U: 'static>(&self, width: u32, height: u32, format: Format, usage: BufferObjectFlags) -> IoResult<Surface<U>> {
        let ptr = unsafe { ffi::gbm_surface_create(*self.ffi, width, height, format as u32, usage.bits()) };
        if ptr.is_null() {
            Err(IoError::last_os_error())
        } else {
    ...
    pub fn create_surface_with_modifiers<U: 'static>(
        ...
        let mods = modifiers.map(Into::into).collect::<Vec<u64>>();
        let ptr = unsafe {
            ffi::gbm_surface_create_with_modifiers(
                *self.ffi,
                ...
                format as u32,
                mods.as_ptr(),
                u32::try_from(mods.len()).expect("Failed to cast u32 from i32"),
            )
        };
        if ptr.is_null() {
            Err(IoError::last_os_error())
        ...
        let ptr = unsafe {
            ffi::gbm_surface_create_with_modifiers2(
                *self.ffi,
                ...
                format as u32,
                mods.as_ptr(),
                u32::try_from(mods.len()).expect("Failed to cast u32 from usize"),
                usage.bits(),
            )
        };
        if ptr.is_null() {
    ...
    pub fn create_buffer_object<U: 'static>(
        ...
        let ptr = unsafe { ffi::gbm_bo_create(*self.ffi, width, height, format as u32, usage.bits()) };
        if ptr.is_null() {
            Err(IoError::last_os_error())
        } else {
    ...
    pub unsafe fn import_buffer_object_from_egl<U: 'static>(
        ...
        let ptr = ffi::gbm_bo_import(*self.ffi, ffi::GBM_BO_IMPORT_EGL_IMAGE, buffer, usage.bits());
        if ptr.is_null() {
            Err(IoError::last_os_error())
        } else {
        ...
        let ptr = unsafe {
            ffi::gbm_bo_import(
                *self.ffi,
                ...
                std::ptr::from_mut::<ffi::gbm_import_fd_data>(&mut fd_data).cast(),
                usage.bits(),
            )
        };
        if ptr.is_null() {
        ...
        let ptr = unsafe {
            ffi::gbm_bo_import(
                *self.ffi,
                ...
                std::ptr::from_mut::<ffi::gbm_import_fd_modifier_data>(&mut fd_data).cast(),
                usage.bits(),
            )
        };
        if ptr.is_null() {
