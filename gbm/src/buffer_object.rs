...
pub struct BufferObject<T: 'static> {
    ...
    pub(crate) device: WeakPtr<ffi::gbm_device>,
    pub(crate) userdata: PhantomData<T>,
}
...
impl<T> fmt::Debug for BufferObject<T> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("BufferObject")
            ...
            .field("device", &format_args!("{:p}", &self.device))
...
impl<T: 'static> BufferObject<T> {
    ...
    pub fn width(&self) -> Result<u32, DeviceDestroyedError> {
        self.device.upgrade().ok_or(DeviceDestroyedError)?;
    ...
    pub fn format(&self) -> Result<Format, DeviceDestroyedError> {
        self.device.upgrade().ok_or(DeviceDestroyedError)?;
        Ok(Format::try_from(unsafe { ffi::gbm_bo_get_format(*self.ffi) }).expect("libgbm returned invalid buffer format"))
    }
    ...
    pub fn plane_count(&self) -> Result<u32, DeviceDestroyedError> {
        self.device.upgrade().ok_or(DeviceDestroyedError)?;
        Ok(unsafe { u32::try_from(ffi::gbm_bo_get_plane_count(*self.ffi)).expect("Failed to cast u32 from i32") })
    }
    ...
    pub fn device_fd(&self) -> Result<BorrowedFd, DeviceDestroyedError> {
        let device_ptr = self.device.upgrade().ok_or(DeviceDestroyedError)?;
    ...
    {
        let device_ref = self.device.upgrade().ok_or(WrongDeviceError)?;
        ...
        unsafe {
            ...
            if ptr.is_null() {
                ...
                Ok(Ok(f(&MappedBufferObject {
                    bo: BORef::Ref(self),
                    buffer: slice::from_raw_parts_mut(ptr.cast(), (height * stride) as usize),
                    data,
                    stride,
                    height,
                    width,
    ...
    pub fn write(&mut self, buffer: &[u8]) -> Result<IoResult<()>, DeviceDestroyedError> {
        self.device.upgrade().ok_or(DeviceDestroyedError)?;
        let result = unsafe { ffi::gbm_bo_write(*self.ffi, buffer.as_ptr().cast(), buffer.len() as _) };
        if result != 0 {
            Ok(Err(IoError::last_os_error()))
        } else {
            Ok(Ok(()))
    ...
    pub fn set_userdata(&mut self, userdata: T) -> Result<Option<T>, DeviceDestroyedError> {
        ...
        unsafe {
            ffi::gbm_bo_set_user_data(*self.ffi, Box::into_raw(boxed).cast(), Some(destroy::<T>));
        }
    ...
    pub fn take_userdata(&mut self) -> Result<Option<T>, DeviceDestroyedError> {
        ...
        if raw.is_null() {
            ...
            unsafe {
                let boxed = Box::from_raw(raw.cast::<T>());
    ...
    fn handle(&self) -> Handle {
        ...
        unsafe {
            Handle::from(NonZeroU32::new_unchecked(
                self.handle().expect("GbmDevice does not exist anymore").u32_,
            ))
        }
    }
}
